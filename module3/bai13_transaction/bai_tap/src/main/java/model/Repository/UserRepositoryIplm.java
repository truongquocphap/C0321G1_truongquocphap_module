...
public class UserRepositoryIplm implements UserRepository {
    ...
    private static final String EDIT_USERS_STORE= "{CALL edit_user(?,?,?)} ";
    private static final String DELETE_USERS_STORE= "{CALL delete_user(?)} ";
...
// bài tập:my sql stored procedure
    @Override
    ...
    public boolean updateUserSP(User user) {
        ...
        if (connection!=null) {
            try {
                try {
                    statement=connection.prepareStatement(EDIT_USERS_STORE);
                    statement.setString(1,user.getName());
                    statement.setString(2,user.getEmail());
                    statement.setString(3,user.getCountry());
                    statement.setInt(4,user.getId());
    ...
    public boolean deleteUserSP(int  id) {
        ...
        try{
            try {
                statement=connection.prepareStatement(DELETE_USERS_STORE);
                statement.setInt(1,id);
                rowDelete=statement.executeLargeUpdate()>0;
...
//                assign permision to user
            ...
            for (int permisionId : permision ){
                assignment.setInt(1,userId);
                assignment.setInt(1,permisionId);
                assignment.executeUpdate();
            }
            connection.commit();
        }else {
